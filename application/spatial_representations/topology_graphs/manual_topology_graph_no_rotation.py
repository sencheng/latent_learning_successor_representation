...
class ManualTopologyGraphNoRotation(SpatialRepresentation):
    ...
    def set_state_action_state_transition(self):
        '''
        This function sets the state-action-state transition matrix.
        Parameters
        ----------
        None    
        Returns
        ...
        None
        '''
        self.world_sas = np.zeros((len(self.nodes), self.clique_size, len(self.nodes)))
        for node in self.nodes:
            for idx, neighbor in enumerate(node.neighbors):
                if neighbor.index != -1:
                    self.world_sas[node.index][idx][neighbor.index] = 1.0
    
    def set_starting_nodes(self, starting_nodes: list):
        '''
        This function sets the starting nodes of the topology graph.
        ...
        for node_index in starting_nodes:
            self.nodes[node_index].start_node = True
        # self.update_visual_elements()
        #TODO : Test : Remove from class definition if it is only being used for visualization
                    
    def set_goal_nodes(self, goal_nodes: list):
        '''
        This function sets the goal nodes of the topology graph.
